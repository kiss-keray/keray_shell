...
import type { ChannelInstance, ChannelInstanceMoveToWindowPayload } from "@/stores/channelInstances";
...
export async function openOrFocusSettingsWindow(tab: SettingsTab = "general"): Promise<void> {
    ...
    await createNewWindow(label, "settings", `&tab=${tab}`, { width: 760, height: 620 }, {});
}
...
export async function openOrFocusServerTreeWindow(): Promise<void> {
    ...
    await createNewWindow(label, "server-tree", `&from=${getCurrentWindow().label}`, { width: 1000, height: 600 }, {});
}
...
export async function openOrFocusEditServerWindow(payload: Omit<EditServerWindowPayload, "from">): Promise<void> {
    ...
    const data: EditServerWindowPayload = { ...payload, from: getCurrentWindow().label };
    const existing = await WebviewWindow.getByLabel(label);
    if (existing) {
    ...
    await createNewWindow(label, "edit-server", "", { width: 720, height: 680, minWidth: 620, minHeight: 560 }, data);
}
...
export async function openOrFocusMonacoEditorWindow(payload: MonacoEditorWindowPayload | MonacoEditorWindowPayload[]): Promise<void> {
    ...
    await createNewWindow(`monaco-editor-${uuid()}`, "monaco-editor", "", { width: 800, height: 680 }, payload, false);
}
...
export async function openUploadConflictWindow(payload: Omit<UploadConflictWindowPayload, "from">): Promise<UploadConflictResolvedPayload> {
    return new Promise(async (resolve) => {
        const data: UploadConflictWindowPayload = { ...payload, from: getCurrentWindow().label };
        ...
        if (existing) {
            ...
            const win = await createNewWindow(label, "upload-conflict", "", { width: 520, height: 280, resizable: true }, data);
            await win.show();
            await win.setFocus();
        }
    });
...
async function createNewWindow(
    ...
    once<AppStartOkPayload>(APP_START_OK_EVENT, (e) => {
        ...
        emitTo({ kind: "Window", label }, WEBVIEW_INIT_DATA_EVENT, data).catch((err) => {
            console.error("send WEBVIEW_INIT_DATA_EVENT error", err);
        });
    });
    ...
    if (osType() === "macos" && disable_native_fullscreen) {
